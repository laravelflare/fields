        ...
        <div class="form-group @if ($errors->has($attribute)) has-error @endif">
            ...
            <label class="control-label" for="{{ $attribute }}">
                ...
                @if (isset($options['required']))
                    <span title="" data-placement="right" data-toggle="tooltip"
                          data-original-title="This field is required">*</span>
                @endif
                @if(isset($options['tooltip']))
                    <span title="" data-placement="right" data-toggle="tooltip" class="badge bg-black"
                          data-original-title="{{ $options['tooltip'] }}">?</span>
                @endif
            </label>
                ...
                >
                    ...
                    @foreach ($options['options'] as $optionValue => $option)
                        <option value="{{ $optionValue }}"
                                @if ($value instanceof \Illuminate\Database\Eloquent\Model && $optionValue == $value->getKey())
                                    selected="selected"
                                @endif
                        >{{ $option }}</option>
                    @endforeach
